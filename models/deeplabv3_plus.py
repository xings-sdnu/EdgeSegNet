...
class ResNet(nn.Module):
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet101', pretrained=True):
        ...
        checkpoint = torch.load(r"D:\MyworkSpace\segmentation\pytorch-segmentation-master\saved\PSPNet\best_model.pth")
...
class Decoder(nn.Module):
    ...
    def forward(self, x, low_level_features):
        ...
        # low_level_features 2,48,129,129
...
class DeepLab(BaseModel):
    ...
    def forward(self, x):
        ...
        # print(x.shape)  # torch.Size([2, 2048, 33, 33])
        x = self.ASSP(x)
        # print(x.shape)  # torch.Size([2, 256, 33, 33])
