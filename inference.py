...
import dataloaders
import models
from get_metrix import compute_mIoU
...
def multi_scale_predict(model, image, scales, num_classes, device, flip=False):
    ...
    edge = np.zeros((num_classes, image.size(2), image.size(3)))

    image = image.data.data.cpu().numpy()
    for scale in scales:
        scaled_img = ndimage.zoom(image, (1.0, 1.0, float(scale), float(scale)), order=1, prefilter=False)
        scaled_img = torch.from_numpy(scaled_img).to(device)
        scaled_img, edge = model(scaled_img)
        scaled_prediction = upsample(scaled_img).cpu()
        edge = upsample(edge).cpu()
        ...
        edge += edge.data.cpu().numpy().squeeze(0)

    total_predictions /= len(scales)
    edge /= len(scales)
    return total_predictions, edge
...
def save_images_edge(image, mask, output_path, image_file, palette):
    # Saves the image, the model output and the results after the post processing
    w, h = image.size
    image_file = os.path.basename(image_file).split('.')[0]
    colorized_mask = colorize_mask(mask, palette)
    colorized_mask.save(os.path.join(output_path, image_file + '_edge.png'))


def _get_seg_metrics(total_correct=0, total_label=0, total_inter=0, total_union=0):
...
def update_seg_metrics(correct, labeled, inter, union, total_correct=0, total_label=0, total_inter=0,
                       total_union=0):
    ...
    with torch.no_grad():
        ...
        for img_file in tbar:
            ...
            if args.mode == 'multiscale':
                prediction, edge = multi_scale_predict(model, input, scales, num_classes, device)
            ...
            # edge = F.softmax(edge, dim=0).argmax(0).cpu().numpy()
            edge = (edge - edge.min()) / (edge.max() - edge.min() + 1e-8)
            # save_images(image, prediction, args.output, img_file, palette)
            cv2.imwrite(args.output + os.path.basename(img_file).split('.')[0] + '_edge.png', edge * 255)
            tbar.set_description(f'fps = {fps}')
            # save_images_edge(image, edge, args.output, img_file, palette)
...
def parse_arguments():
    ...
    parser.add_argument('-m', '--model', default='saved/DeepLab/best_model.pth', type=str,
